...
class KalmanFilterGUI(QWidget):
    ...
    def select_file(self):
        file_name, _ = QFileDialog.getOpenFileName(self, "Select Input File", "", "CSV Files (*.csv)")
        if file_name:
            self.file_label.setText("File selected: " + file_name)
    ...
    def process_data(self):
        ...
        if not input_file:
            self.text_output.append("Please select an input file.")
            return

        self.text_output.append(f"Processing with:\nInput File: {input_file}\nTrack Mode: {track_mode}\nFilter Option: {filter_option}\nAssociation Type: {association_type}")
        ...
        if self.tracks is None:
            self.text_output.append("No tracks were generated.")
        else:
            self.text_output.append(f"Number of tracks: {len(self.tracks)}")
        ...
        for track in self.tracks:
            for measurement, _ in track['measurements']:
                self.plot_data['times'].append(measurement[3])  # Assuming time is at index 3
                x, y, z = sph2cart(*measurement[:3])
                self.plot_data['measurements_x'].append(x)
                self.plot_data['measurements_y'].append(y)
                self.plot_data['measurements_z'].append(z)
                self.plot_data['Sf_x'].append(track['Sf'][0, 0])
                self.plot_data['Sf_y'].append(track['Sf'][1, 0])
                self.plot_data['Sf_z'].append(track['Sf'][2, 0])
    ...
    def update_plot(self):
        ...
        if not self.plot_data or x_axis not in self.plot_data or y_axis not in self.plot_data:
            self.text_output.append("No data available for plotting.")
            return

        self.plot_canvas.figure.clear()
        ax = self.plot_canvas.figure.add_subplot(111)
        ax.plot(self.plot_data[x_axis], self.plot_data[y_axis], 'o-')
        ax.set_xlabel(x_axis)
        ax.set_ylabel(y_axis)
        ax.set_title(f"{y_axis} vs {x_axis}")
        self.plot_canvas.draw()

        self.text_output.append(f"Plot updated: {y_axis} vs {x_axis}")
